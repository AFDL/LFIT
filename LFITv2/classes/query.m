classdef query
    ...
    properties
        ...
        aperature   = 'full';   % aperture enforcing of microlenses: 'full', 'circ'
        ...
        grouping    = 'image';  % directory flag is 0 to save refocused images on a per-image basis or 1 to save on a per-alpha basis (must be constant across requestVector lines)
    ...
    methods
        ...
        function obj = set.aperature( obj, val )
            opts = {'full','circ'};
			if istring(val) && any(strcmpi(val,opts))
                obj.aperature = val;
            else
