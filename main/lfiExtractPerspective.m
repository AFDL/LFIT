function [ slew ] = lfiExtractPerspective( query, lightfield )
%LFIEXTRACTPERSPECTIVE
...
if mod(query.pUV(1),1) || mod(query.pUV(2),1) %ie, if u0 or v0 is not an integer. That's the only way you can really 'supersample' uv, and even so, it's really just interpolation...
    SS = 'both';
end
...
uIndex = -query.pUV(1) + microRadius+1; %u0 is negative since the uVector decreases from left to right (ie +7 to -7) while MATLAB image indexing increases from top to bottom
vIndex = -query.pUV(2) + microRadius+1; %v0 is negative since the vVector decreases from top to bottom (ie +7 to -7) while MATLAB image indexing increases from top to bottom
...
switch SS
    ...
    case {'uv','both'}
        ...
        [tQuery, sQuery, vQuery, uQuery] = ndgrid(tSSRange,sSSRange,query.pUV(2)*single(sizePixelAperture),query.pUV(1)*single(sizePixelAperture));
